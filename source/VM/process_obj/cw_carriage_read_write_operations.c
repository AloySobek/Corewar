...
void		cw_write_operation(t_process *p_process_instance, t_arena *p_arena_obj, t_buffer *pBufferObj, int inputArg)
{
	...
	else if (inputArg == CW_IND_CODE)
	{
		...
		if (p_process_instance->p_current_command->id != CW_LLD)
		...
		p_process_instance->cw_save_pos(p_process_instance, CW_ADDIT_SAVE);
		...
		p_process_instance->cw_carriage_return(p_process_instance, CW_ADDIT_SAVE);
	}
	else
