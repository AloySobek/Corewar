...
/*   Updated: 2019/11/09 17:22:02 by vrichese         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "corewar.h"

static void	cw_reg_check(t_process *p_process_instance, t_arena *p_arena_obj, int type, int *found)
{
	if (!*found)
		return ;
	if (type == CW_REG_CODE)
...
static void	cw_copy_reg(t_process *p_process_instance, t_process *p_copying_carriage)
{
	int		iter;

	iter = CW_ITERATOR;
	while (++iter < CW_REG_SIZE * CW_REG_L)
...
extern void	cw_process_registers_functions_linker(t_process *p_process_instance)
{
	...
	p_process_instance->cw_reg_check = cw_reg_check;
}